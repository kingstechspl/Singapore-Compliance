...
def get_journal_entry_data(sgst, filters):
	...
	conditions = (
		"(jea.account = %(bank_interest)s OR jea.account = %(exchange_gain)s)"
		" AND je.docstatus = 1"
	)
...
def get_payment_entry_data(sgst, filters):
	...
	conditions = (
		"(ped.account = %(bank_interest)s OR ped.account = %(exchange_gain)s)"
		" AND pe.docstatus = 1"
	)
	...
	total = sum(row.get("amount") for row in py_data)
...
def get_purchase_invoice_data(sgst, filters):
	...
	conditions = f"pt.parent = p.name AND p.docstatus = 1 AND pt.parenttype = 'Purchase Invoice' AND pt.account_head IN ({placeholders})"
...
def get_data(filters=None):
	...
	if purchase_invoice_with_tax_total and sales_invoice_with_tax_total:
		...
		if diff > 0:
			label = "<b>Box 8 Tax To Be Paid</b>"
			amount = diff
		elif diff < 0:
			label = "<b>Box 8 Tax To Be Claimed</b>"
			amount = -diff
		else:
			label = "<b>Box 8 Tax</b>"
